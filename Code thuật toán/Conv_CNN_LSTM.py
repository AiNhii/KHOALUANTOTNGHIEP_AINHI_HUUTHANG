...
from keras.models import Sequential
from keras.layers import ConvLSTM2D, TimeDistributed, Conv1D, Flatten, Dense

def create_model(n_seq, n_steps, n_features):
    model = Sequential()

    model.add(ConvLSTM2D(filters=64, kernel_size=(1, 2), activation='relu', input_shape=(n_seq, 1, n_steps, n_features), return_sequences=True))
    model.add(ConvLSTM2D(filters=128, kernel_size=(1, 2), activation='relu', return_sequences=True))
...
final_model = create_model(n_seq = best_params['n_seq'], n_features = best_params['n_features'], n_steps = best_params['n_steps'])
